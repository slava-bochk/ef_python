...
def main():
    ...
    if is_config:
        ...
        Runner(sim, solver_class(sim.spat_mesh.mesh, sim.inner_regions), writer).start()
    else:
        ...
        Runner(sim, solver_class(sim.spat_mesh.mesh, sim.inner_regions), writer).continue_()
    del sim
    return 0
